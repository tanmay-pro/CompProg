...
struct segTree
{
    ...
    {
        item temp;
        temp.sumSeg = a.sumSeg + b.sumSeg;
        temp.maxSubSeg = max({a.maxSubSeg, b.maxSubSeg, a.suffix + b.prefix});
        temp.prefix = max(a.prefix, a.sumSeg + b.prefix);
        temp.suffix = max(b.suffix, b.sumSeg + a.suffix);
        return temp;
    }
...
int main()
{
    ...
    st.init(n);
    st.build(a);
    item temp = st.calcFunc(0, n);
    if(temp.maxSubSeg > 0)
        cout << temp.maxSubSeg << endl;
    else
        cout << 0 << endl;
    fo(i, m)
    {
        ll index, val;
        cin >> index >> val;
        st.updateElem(index, val);
        temp = st.calcFunc(0, n);
        if(temp.maxSubSeg > 0)
            cout << temp.maxSubSeg << endl;
        else
            cout << 0 << endl;
