...
struct segTree
{
    ...
    void init(ll n) // Create Nodes Array
    {
        ...
        nodes.resize(2 * size);
        neutralElement = giveValue(LONG_LONG_MIN);
    }

    ll firstAbove(ll val, ll minInd, ll x, ll lx, ll rx)
    {
        if (nodes[x].val < val)
        {
            return -1;
        ...
        ll mid = (lx + rx) / 2;
        ll s1 = firstAbove(val, minInd,  2 * x + 1, lx, mid);
        if (s1 != -1)
        {
            return s1;
        }
        return firstAbove(val, minInd, 2 * x + 2, mid, rx);
    }


    ll firstAbove(ll val, ll minInd)
    {
        return firstAbove(val, minInd, 0, 0, size);
    }
...
int main()
{
    ...
    fo(i, m)
    {
        ...
        if(op == 1)
        {
            st.updateElem(ind, val);
        }
        else
        {
            ll ans = st.firstAbove(ind, val);
            cout << ans;
            br;
        }
    }
