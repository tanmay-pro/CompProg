...
struct segTree
{
    ...
    ll firstAbove(ll val, ll x, ll lx, ll rx)
    {
        if (nodes[x].val < val)
        {
            return -1;
        ...
        ll mid = (lx + rx) / 2;
        ll s1 = firstAbove(val, 2 * x + 1, lx, mid);
        if (s1 != -1)
        {
            return s1;
        }
        return firstAbove(val, 2 * x + 2, mid, rx);
    }


    ll firstAbove(ll val)
    {
        return firstAbove(val, 0, 0, size);
    }
...
int main()
{
    ...
    fo(i, m)
    {
        ...
        if(op == 1)
        {
            ll ind;
            cin >> ind;
            st.updateElem(val, ind);
        }
        else
        {
            ll ans = st.firstAbove(val);
            cout << ans;
            br;
        }
    }
